...
#AutoIt3Wrapper_Outfile_x64=autoArmory.exe
...
Global $leaveGameOnNormalRift = 1 		; Should we leave game on rifts? No effect on grifts
Global $riftForPools = 1				; When true we rift until pool marker shows on the right, then run $numRiftsToRun more rifts
...
;===============================================================================
; Description:      loadSettings - Loads settings from ini file.
; Parameter(s):  	None
; Return Value(s):  None
;===============================================================================
Func loadSettings()
	$numRiftsToRun = number(readSetting("General", "numRiftsToRun", "1"))
	$numGriftsToRun = number(readSetting("General", "numGriftsToRun", "2"))
	$leaveGameOnNormalRift = number(readSetting("General", "leaveGameOnNormalRift", "1"))
	$riftForPools = number(readSetting("General", "riftForPools", "1"))
EndFunc

;===============================================================================
; Description:      testSwapGear - performs the gear swap script for setup
;                   testing purposes. Press Ctrl + g to swap back and forth
; Parameter(s):  	None
; Return Value(s):  None
;===============================================================================
Func testSwapGear()
...
Func aaLog($functionName, $message, $state = 0, $settings = 0)
	...
	If $state Then
		...
		;_FileWriteLog($autoArmoryLogFile, "   | $p: " & $pools & " | $rT: " & $riftType & " | $rC: " & $riftCount & " | $cgf: " & $changingGearFlag & " | $jfgs: " & $justFailedGearSwap & " | $rbwt: " & $rosbotwindowtitle)
	EndIf
	...
	If $settings Then
		...
		;_FileWriteLog($autoArmoryLogFile, "   | nrtr: " & $numRiftsToRun & " | ngrtr: " & $numGriftsToRun & " | lgonr: " & $leaveGameOnNormalRift & " | rfp: " & $riftForPools)
	EndIf
EndFunc
...
Func incRiftCount()
	...
	; Increment if we have pools or if we are not running RFP
	If ($riftForPools And $pools) Then
		$riftCount += 1
		aaLog("incRiftCount", "RFP riftCount ++",1,1)
	ElseIf Not $riftForPools Then
		$riftCount += 1
		aaLog("incRiftCount", $numRiftsToRun & ":" & $numGriftsToRun & " riftCount ++",1,1)
	EndIf

	; Update our pools
	If $riftForPools Then
		$pools = poolOrNah()
		aaLog("incRiftCount", "setting pools flag to: " & $pools,1,1)
	EndIf
...
Func armoryControler()
    ...
    ; Should we switch rift types, we need the right number of rifts, pools if rfp
    If ($changingGearFlag = 0 And (($riftType = 1 And $riftCount >= $numRiftsToRun And ($pools Or Not $riftForPools)) Or (($riftType = 2 And Not $pools And $riftForPools) Or (Not $riftForPools And $riftCount >= $numGriftsToRun)))) Then
