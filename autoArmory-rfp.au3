...
Func poolOrNah()
	...
	_FileWriteLog($hFile, "[poolOrNah() - checking pools]")

	; Activate window
	WinActivate("[CLASS:D3 Main Window Class]")
		If @error Then
		_FileWriteLog($hFile, "[poolOrNah() - ERROR] WinActivate Failed, Diablo Window not Present")
		Return SetError(1,0,0)
	EndIf
	...
	If @error Then
		_FileWriteLog($hFile, "[poolOrNah() - ERROR] WinGetPos Failed, Diablo Window not Present")
		Return SetError(1,0,0)
	EndIf
	...
	_FileWriteLog($hFile, "[poolOrNah() - done]  $poolCount: " & $poolCount)

	If ($poolCount > 0) Then
		_FileWriteLog($hFile, "[$poolCount() - We have pools")
		Return True
	Else
		_FileWriteLog($hFile, "[$poolCount() - No winner]  ")
		Return False
	EndIf
EndFunc

; Increment rift count when we have pools
Func incRiftCount()
	_FileWriteLog($hFile, "[incRiftCount() call]  pool: " & $pools & " type: " & $riftType & "  count: " & $riftCount & "  $riftsToRun: " & $numRiftsToRun)

	If ($pools = 1) Then
		$riftCount += 1
		_FileWriteLog($hFile, "[incRiftCount() riftcount ++]  pool: " & $pools & " type: " & $riftType & "  count: " & $riftCount & "  $riftsToRun: " & $numRiftsToRun)
	EndIf
...
Func decRiftCount()
	...
	_FileWriteLog($hFile, "[decRiftCount() called]  riftType: " & $riftType & "  riftCount: " & $riftCount & "  $numRiftsToRun: " & $numRiftsToRun)
EndFunc
...
Func armoryControler()
	...
	ElseIf $ignoreCounter > 10 Then
		_FileWriteLog($hFile, "[armoryControler() - called during gear change]  Fixing state...")
		fixState()
		Return
	EndIf
	_FileWriteLog($hFile, "[armoryControler() - called]  riftType: " & $riftType & "  riftCount: " & $riftCount & "  $numRiftsToRun: " & $numRiftsToRun)
    ...
    If (($riftType = 1 And $riftCount >= $numRiftsToRun  And $changingGearFlag = 0 And $pools) Or ($riftType = 2 And $pools = False And $changingGearFlag = 0)) Then
		...
		_FileWriteLog($hFile, "[armoryControler() - clicking return to game)]  1")
		...
		_FileWriteLog($hFile, "[armoryControler() - switching rift/armory] Pools: " & $pools & " riftType: " & $riftType & "  riftCount: " & $riftCount & "  $numRiftsToRun: " & $numRiftsToRun)
		...
		; Click Return to game just in case the run failed
		_FileWriteLog($hFile, "[armoryControler() - clicking return to game)]  2")
        MouseClick("left", 408 + $x, 573 + $y, 1, 1)
		sleep(100)
        MouseClick("left", 408 + $x, 573 + $y, 1, 1)
		sleep(100)
		...
		; Click Return to game just in case the run failed
		_FileWriteLog($hFile, "[armoryControler() - clicking return to game)]  3")
        MouseClick("left", 408 + $x, 573 + $y, 1, 1)
		sleep(100)
        MouseClick("left", 408 + $x, 573 + $y, 1, 1)
		sleep(100)
		...
		If $armNumber = $riftType Then
			_FileWriteLog($hFile, "[armoryControler() - Select Armory worked, continuing to changeBotRiftType()]  riftType: " & $riftType & "  riftCount: " & $riftCount & "  $numRiftsToRun: " & $numRiftsToRun & " armNumber: " & $armNumber)
...
Func changeBotRiftType($riftType)
	_FileWriteLog($hFile, "[changeBotRiftType() - Changing to new rift type in bot]  riftType: " & $riftType & "  riftCount: " & $riftCount & "  $numRiftsToRun: " & $numRiftsToRun & " armNumber: " & $armNumber)
	...
	If $riftType = 1 Then
		_FileWriteLog($hFile, "[changeBotRiftType() - Changing stay in game to 100]  riftType: " & $riftType & "  riftCount: " & $riftCount & "  $numRiftsToRun: " & $numRiftsToRun & " armNumber: " & $armNumber)
